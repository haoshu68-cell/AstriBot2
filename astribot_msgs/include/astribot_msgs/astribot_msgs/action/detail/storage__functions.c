// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from astribot_msgs:action/Storage.idl
// generated code does not contain a copyright notice
#include "astribot_msgs/action/detail/storage__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `topic_list`
// Member `archive_path`
#include "rosidl_runtime_c/string_functions.h"

bool
astribot_msgs__action__Storage_Goal__init(astribot_msgs__action__Storage_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // topic_list
  if (!rosidl_runtime_c__String__init(&msg->topic_list)) {
    astribot_msgs__action__Storage_Goal__fini(msg);
    return false;
  }
  // storage_type
  // trigger_time
  // archive_path
  if (!rosidl_runtime_c__String__init(&msg->archive_path)) {
    astribot_msgs__action__Storage_Goal__fini(msg);
    return false;
  }
  return true;
}

void
astribot_msgs__action__Storage_Goal__fini(astribot_msgs__action__Storage_Goal * msg)
{
  if (!msg) {
    return;
  }
  // topic_list
  rosidl_runtime_c__String__fini(&msg->topic_list);
  // storage_type
  // trigger_time
  // archive_path
  rosidl_runtime_c__String__fini(&msg->archive_path);
}

bool
astribot_msgs__action__Storage_Goal__are_equal(const astribot_msgs__action__Storage_Goal * lhs, const astribot_msgs__action__Storage_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // topic_list
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->topic_list), &(rhs->topic_list)))
  {
    return false;
  }
  // storage_type
  if (lhs->storage_type != rhs->storage_type) {
    return false;
  }
  // trigger_time
  if (lhs->trigger_time != rhs->trigger_time) {
    return false;
  }
  // archive_path
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->archive_path), &(rhs->archive_path)))
  {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_Goal__copy(
  const astribot_msgs__action__Storage_Goal * input,
  astribot_msgs__action__Storage_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // topic_list
  if (!rosidl_runtime_c__String__copy(
      &(input->topic_list), &(output->topic_list)))
  {
    return false;
  }
  // storage_type
  output->storage_type = input->storage_type;
  // trigger_time
  output->trigger_time = input->trigger_time;
  // archive_path
  if (!rosidl_runtime_c__String__copy(
      &(input->archive_path), &(output->archive_path)))
  {
    return false;
  }
  return true;
}

astribot_msgs__action__Storage_Goal *
astribot_msgs__action__Storage_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Goal * msg = (astribot_msgs__action__Storage_Goal *)allocator.allocate(sizeof(astribot_msgs__action__Storage_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_Goal));
  bool success = astribot_msgs__action__Storage_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_Goal__destroy(astribot_msgs__action__Storage_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_Goal__Sequence__init(astribot_msgs__action__Storage_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Goal * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_Goal *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_Goal__Sequence__fini(astribot_msgs__action__Storage_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_Goal__Sequence *
astribot_msgs__action__Storage_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Goal__Sequence * array = (astribot_msgs__action__Storage_Goal__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_Goal__Sequence__destroy(astribot_msgs__action__Storage_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_Goal__Sequence__are_equal(const astribot_msgs__action__Storage_Goal__Sequence * lhs, const astribot_msgs__action__Storage_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_Goal__Sequence__copy(
  const astribot_msgs__action__Storage_Goal__Sequence * input,
  astribot_msgs__action__Storage_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_Goal * data =
      (astribot_msgs__action__Storage_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `dir_path`
// Member `miss_topic_list`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
astribot_msgs__action__Storage_Result__init(astribot_msgs__action__Storage_Result * msg)
{
  if (!msg) {
    return false;
  }
  // dir_path
  if (!rosidl_runtime_c__String__init(&msg->dir_path)) {
    astribot_msgs__action__Storage_Result__fini(msg);
    return false;
  }
  // miss_topic_list
  if (!rosidl_runtime_c__String__init(&msg->miss_topic_list)) {
    astribot_msgs__action__Storage_Result__fini(msg);
    return false;
  }
  return true;
}

void
astribot_msgs__action__Storage_Result__fini(astribot_msgs__action__Storage_Result * msg)
{
  if (!msg) {
    return;
  }
  // dir_path
  rosidl_runtime_c__String__fini(&msg->dir_path);
  // miss_topic_list
  rosidl_runtime_c__String__fini(&msg->miss_topic_list);
}

bool
astribot_msgs__action__Storage_Result__are_equal(const astribot_msgs__action__Storage_Result * lhs, const astribot_msgs__action__Storage_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // dir_path
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->dir_path), &(rhs->dir_path)))
  {
    return false;
  }
  // miss_topic_list
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->miss_topic_list), &(rhs->miss_topic_list)))
  {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_Result__copy(
  const astribot_msgs__action__Storage_Result * input,
  astribot_msgs__action__Storage_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // dir_path
  if (!rosidl_runtime_c__String__copy(
      &(input->dir_path), &(output->dir_path)))
  {
    return false;
  }
  // miss_topic_list
  if (!rosidl_runtime_c__String__copy(
      &(input->miss_topic_list), &(output->miss_topic_list)))
  {
    return false;
  }
  return true;
}

astribot_msgs__action__Storage_Result *
astribot_msgs__action__Storage_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Result * msg = (astribot_msgs__action__Storage_Result *)allocator.allocate(sizeof(astribot_msgs__action__Storage_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_Result));
  bool success = astribot_msgs__action__Storage_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_Result__destroy(astribot_msgs__action__Storage_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_Result__Sequence__init(astribot_msgs__action__Storage_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Result * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_Result *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_Result__Sequence__fini(astribot_msgs__action__Storage_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_Result__Sequence *
astribot_msgs__action__Storage_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Result__Sequence * array = (astribot_msgs__action__Storage_Result__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_Result__Sequence__destroy(astribot_msgs__action__Storage_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_Result__Sequence__are_equal(const astribot_msgs__action__Storage_Result__Sequence * lhs, const astribot_msgs__action__Storage_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_Result__Sequence__copy(
  const astribot_msgs__action__Storage_Result__Sequence * input,
  astribot_msgs__action__Storage_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_Result * data =
      (astribot_msgs__action__Storage_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `miss_topic_list`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
astribot_msgs__action__Storage_Feedback__init(astribot_msgs__action__Storage_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // seconds
  // disk_space
  // miss_topic_list
  if (!rosidl_runtime_c__String__init(&msg->miss_topic_list)) {
    astribot_msgs__action__Storage_Feedback__fini(msg);
    return false;
  }
  // status
  return true;
}

void
astribot_msgs__action__Storage_Feedback__fini(astribot_msgs__action__Storage_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // seconds
  // disk_space
  // miss_topic_list
  rosidl_runtime_c__String__fini(&msg->miss_topic_list);
  // status
}

bool
astribot_msgs__action__Storage_Feedback__are_equal(const astribot_msgs__action__Storage_Feedback * lhs, const astribot_msgs__action__Storage_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // seconds
  if (lhs->seconds != rhs->seconds) {
    return false;
  }
  // disk_space
  if (lhs->disk_space != rhs->disk_space) {
    return false;
  }
  // miss_topic_list
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->miss_topic_list), &(rhs->miss_topic_list)))
  {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_Feedback__copy(
  const astribot_msgs__action__Storage_Feedback * input,
  astribot_msgs__action__Storage_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // seconds
  output->seconds = input->seconds;
  // disk_space
  output->disk_space = input->disk_space;
  // miss_topic_list
  if (!rosidl_runtime_c__String__copy(
      &(input->miss_topic_list), &(output->miss_topic_list)))
  {
    return false;
  }
  // status
  output->status = input->status;
  return true;
}

astribot_msgs__action__Storage_Feedback *
astribot_msgs__action__Storage_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Feedback * msg = (astribot_msgs__action__Storage_Feedback *)allocator.allocate(sizeof(astribot_msgs__action__Storage_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_Feedback));
  bool success = astribot_msgs__action__Storage_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_Feedback__destroy(astribot_msgs__action__Storage_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_Feedback__Sequence__init(astribot_msgs__action__Storage_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Feedback * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_Feedback *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_Feedback__Sequence__fini(astribot_msgs__action__Storage_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_Feedback__Sequence *
astribot_msgs__action__Storage_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_Feedback__Sequence * array = (astribot_msgs__action__Storage_Feedback__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_Feedback__Sequence__destroy(astribot_msgs__action__Storage_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_Feedback__Sequence__are_equal(const astribot_msgs__action__Storage_Feedback__Sequence * lhs, const astribot_msgs__action__Storage_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_Feedback__Sequence__copy(
  const astribot_msgs__action__Storage_Feedback__Sequence * input,
  astribot_msgs__action__Storage_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_Feedback * data =
      (astribot_msgs__action__Storage_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "astribot_msgs/action/detail/storage__functions.h"

bool
astribot_msgs__action__Storage_SendGoal_Request__init(astribot_msgs__action__Storage_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    astribot_msgs__action__Storage_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!astribot_msgs__action__Storage_Goal__init(&msg->goal)) {
    astribot_msgs__action__Storage_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
astribot_msgs__action__Storage_SendGoal_Request__fini(astribot_msgs__action__Storage_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  astribot_msgs__action__Storage_Goal__fini(&msg->goal);
}

bool
astribot_msgs__action__Storage_SendGoal_Request__are_equal(const astribot_msgs__action__Storage_SendGoal_Request * lhs, const astribot_msgs__action__Storage_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!astribot_msgs__action__Storage_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_SendGoal_Request__copy(
  const astribot_msgs__action__Storage_SendGoal_Request * input,
  astribot_msgs__action__Storage_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!astribot_msgs__action__Storage_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

astribot_msgs__action__Storage_SendGoal_Request *
astribot_msgs__action__Storage_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_SendGoal_Request * msg = (astribot_msgs__action__Storage_SendGoal_Request *)allocator.allocate(sizeof(astribot_msgs__action__Storage_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_SendGoal_Request));
  bool success = astribot_msgs__action__Storage_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_SendGoal_Request__destroy(astribot_msgs__action__Storage_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_SendGoal_Request__Sequence__init(astribot_msgs__action__Storage_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_SendGoal_Request * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_SendGoal_Request *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_SendGoal_Request__Sequence__fini(astribot_msgs__action__Storage_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_SendGoal_Request__Sequence *
astribot_msgs__action__Storage_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_SendGoal_Request__Sequence * array = (astribot_msgs__action__Storage_SendGoal_Request__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_SendGoal_Request__Sequence__destroy(astribot_msgs__action__Storage_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_SendGoal_Request__Sequence__are_equal(const astribot_msgs__action__Storage_SendGoal_Request__Sequence * lhs, const astribot_msgs__action__Storage_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_SendGoal_Request__Sequence__copy(
  const astribot_msgs__action__Storage_SendGoal_Request__Sequence * input,
  astribot_msgs__action__Storage_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_SendGoal_Request * data =
      (astribot_msgs__action__Storage_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
astribot_msgs__action__Storage_SendGoal_Response__init(astribot_msgs__action__Storage_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    astribot_msgs__action__Storage_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
astribot_msgs__action__Storage_SendGoal_Response__fini(astribot_msgs__action__Storage_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
astribot_msgs__action__Storage_SendGoal_Response__are_equal(const astribot_msgs__action__Storage_SendGoal_Response * lhs, const astribot_msgs__action__Storage_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_SendGoal_Response__copy(
  const astribot_msgs__action__Storage_SendGoal_Response * input,
  astribot_msgs__action__Storage_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

astribot_msgs__action__Storage_SendGoal_Response *
astribot_msgs__action__Storage_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_SendGoal_Response * msg = (astribot_msgs__action__Storage_SendGoal_Response *)allocator.allocate(sizeof(astribot_msgs__action__Storage_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_SendGoal_Response));
  bool success = astribot_msgs__action__Storage_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_SendGoal_Response__destroy(astribot_msgs__action__Storage_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_SendGoal_Response__Sequence__init(astribot_msgs__action__Storage_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_SendGoal_Response * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_SendGoal_Response *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_SendGoal_Response__Sequence__fini(astribot_msgs__action__Storage_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_SendGoal_Response__Sequence *
astribot_msgs__action__Storage_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_SendGoal_Response__Sequence * array = (astribot_msgs__action__Storage_SendGoal_Response__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_SendGoal_Response__Sequence__destroy(astribot_msgs__action__Storage_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_SendGoal_Response__Sequence__are_equal(const astribot_msgs__action__Storage_SendGoal_Response__Sequence * lhs, const astribot_msgs__action__Storage_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_SendGoal_Response__Sequence__copy(
  const astribot_msgs__action__Storage_SendGoal_Response__Sequence * input,
  astribot_msgs__action__Storage_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_SendGoal_Response * data =
      (astribot_msgs__action__Storage_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
astribot_msgs__action__Storage_GetResult_Request__init(astribot_msgs__action__Storage_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    astribot_msgs__action__Storage_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
astribot_msgs__action__Storage_GetResult_Request__fini(astribot_msgs__action__Storage_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
astribot_msgs__action__Storage_GetResult_Request__are_equal(const astribot_msgs__action__Storage_GetResult_Request * lhs, const astribot_msgs__action__Storage_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_GetResult_Request__copy(
  const astribot_msgs__action__Storage_GetResult_Request * input,
  astribot_msgs__action__Storage_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

astribot_msgs__action__Storage_GetResult_Request *
astribot_msgs__action__Storage_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_GetResult_Request * msg = (astribot_msgs__action__Storage_GetResult_Request *)allocator.allocate(sizeof(astribot_msgs__action__Storage_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_GetResult_Request));
  bool success = astribot_msgs__action__Storage_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_GetResult_Request__destroy(astribot_msgs__action__Storage_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_GetResult_Request__Sequence__init(astribot_msgs__action__Storage_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_GetResult_Request * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_GetResult_Request *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_GetResult_Request__Sequence__fini(astribot_msgs__action__Storage_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_GetResult_Request__Sequence *
astribot_msgs__action__Storage_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_GetResult_Request__Sequence * array = (astribot_msgs__action__Storage_GetResult_Request__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_GetResult_Request__Sequence__destroy(astribot_msgs__action__Storage_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_GetResult_Request__Sequence__are_equal(const astribot_msgs__action__Storage_GetResult_Request__Sequence * lhs, const astribot_msgs__action__Storage_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_GetResult_Request__Sequence__copy(
  const astribot_msgs__action__Storage_GetResult_Request__Sequence * input,
  astribot_msgs__action__Storage_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_GetResult_Request * data =
      (astribot_msgs__action__Storage_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "astribot_msgs/action/detail/storage__functions.h"

bool
astribot_msgs__action__Storage_GetResult_Response__init(astribot_msgs__action__Storage_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!astribot_msgs__action__Storage_Result__init(&msg->result)) {
    astribot_msgs__action__Storage_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
astribot_msgs__action__Storage_GetResult_Response__fini(astribot_msgs__action__Storage_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  astribot_msgs__action__Storage_Result__fini(&msg->result);
}

bool
astribot_msgs__action__Storage_GetResult_Response__are_equal(const astribot_msgs__action__Storage_GetResult_Response * lhs, const astribot_msgs__action__Storage_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!astribot_msgs__action__Storage_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_GetResult_Response__copy(
  const astribot_msgs__action__Storage_GetResult_Response * input,
  astribot_msgs__action__Storage_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!astribot_msgs__action__Storage_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

astribot_msgs__action__Storage_GetResult_Response *
astribot_msgs__action__Storage_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_GetResult_Response * msg = (astribot_msgs__action__Storage_GetResult_Response *)allocator.allocate(sizeof(astribot_msgs__action__Storage_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_GetResult_Response));
  bool success = astribot_msgs__action__Storage_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_GetResult_Response__destroy(astribot_msgs__action__Storage_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_GetResult_Response__Sequence__init(astribot_msgs__action__Storage_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_GetResult_Response * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_GetResult_Response *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_GetResult_Response__Sequence__fini(astribot_msgs__action__Storage_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_GetResult_Response__Sequence *
astribot_msgs__action__Storage_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_GetResult_Response__Sequence * array = (astribot_msgs__action__Storage_GetResult_Response__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_GetResult_Response__Sequence__destroy(astribot_msgs__action__Storage_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_GetResult_Response__Sequence__are_equal(const astribot_msgs__action__Storage_GetResult_Response__Sequence * lhs, const astribot_msgs__action__Storage_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_GetResult_Response__Sequence__copy(
  const astribot_msgs__action__Storage_GetResult_Response__Sequence * input,
  astribot_msgs__action__Storage_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_GetResult_Response * data =
      (astribot_msgs__action__Storage_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "astribot_msgs/action/detail/storage__functions.h"

bool
astribot_msgs__action__Storage_FeedbackMessage__init(astribot_msgs__action__Storage_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    astribot_msgs__action__Storage_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!astribot_msgs__action__Storage_Feedback__init(&msg->feedback)) {
    astribot_msgs__action__Storage_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
astribot_msgs__action__Storage_FeedbackMessage__fini(astribot_msgs__action__Storage_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  astribot_msgs__action__Storage_Feedback__fini(&msg->feedback);
}

bool
astribot_msgs__action__Storage_FeedbackMessage__are_equal(const astribot_msgs__action__Storage_FeedbackMessage * lhs, const astribot_msgs__action__Storage_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!astribot_msgs__action__Storage_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
astribot_msgs__action__Storage_FeedbackMessage__copy(
  const astribot_msgs__action__Storage_FeedbackMessage * input,
  astribot_msgs__action__Storage_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!astribot_msgs__action__Storage_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

astribot_msgs__action__Storage_FeedbackMessage *
astribot_msgs__action__Storage_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_FeedbackMessage * msg = (astribot_msgs__action__Storage_FeedbackMessage *)allocator.allocate(sizeof(astribot_msgs__action__Storage_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(astribot_msgs__action__Storage_FeedbackMessage));
  bool success = astribot_msgs__action__Storage_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
astribot_msgs__action__Storage_FeedbackMessage__destroy(astribot_msgs__action__Storage_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    astribot_msgs__action__Storage_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
astribot_msgs__action__Storage_FeedbackMessage__Sequence__init(astribot_msgs__action__Storage_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_FeedbackMessage * data = NULL;

  if (size) {
    data = (astribot_msgs__action__Storage_FeedbackMessage *)allocator.zero_allocate(size, sizeof(astribot_msgs__action__Storage_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = astribot_msgs__action__Storage_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        astribot_msgs__action__Storage_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
astribot_msgs__action__Storage_FeedbackMessage__Sequence__fini(astribot_msgs__action__Storage_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      astribot_msgs__action__Storage_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

astribot_msgs__action__Storage_FeedbackMessage__Sequence *
astribot_msgs__action__Storage_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  astribot_msgs__action__Storage_FeedbackMessage__Sequence * array = (astribot_msgs__action__Storage_FeedbackMessage__Sequence *)allocator.allocate(sizeof(astribot_msgs__action__Storage_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = astribot_msgs__action__Storage_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
astribot_msgs__action__Storage_FeedbackMessage__Sequence__destroy(astribot_msgs__action__Storage_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    astribot_msgs__action__Storage_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
astribot_msgs__action__Storage_FeedbackMessage__Sequence__are_equal(const astribot_msgs__action__Storage_FeedbackMessage__Sequence * lhs, const astribot_msgs__action__Storage_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!astribot_msgs__action__Storage_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
astribot_msgs__action__Storage_FeedbackMessage__Sequence__copy(
  const astribot_msgs__action__Storage_FeedbackMessage__Sequence * input,
  astribot_msgs__action__Storage_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(astribot_msgs__action__Storage_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    astribot_msgs__action__Storage_FeedbackMessage * data =
      (astribot_msgs__action__Storage_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!astribot_msgs__action__Storage_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          astribot_msgs__action__Storage_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!astribot_msgs__action__Storage_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
