...
#
# 口径：局部规划器 MPPI，线速度上限 0.2 m/s，use_sim_time=false，ROS_DOMAIN_ID=25。
#
# ┌───────────────────────────────────────────────────────────────────────────┐
# │ 使能（默认开）做了什么                                                     │
# └───────────────────────────────────────────────────────────────────────────┘
...
PY
  ...
  say "判据 5/7  nav2 活着且吃到地图（MPPI / 0.2m/s / 真墙钟）"
...
PY
  ...
  # 口径已定（用户 2026-09-03 决定）：**0.2 m/s 是各轴独立上限**，不是合速度模长上限。
  #   => vx_max/vy_max 各 0.2 是正确配置；合速度模长上界 sqrt(0.2²+0.2²)=0.283 属**设计内**，
  ...
  local cap_fail=0
  for ax in vx_max vy_max; do
    local vv
    vv=$(ros2 param get /controller_server FollowPath.$ax 2>/dev/null | grep -oE "[0-9.]+" | tail -1 || true)
    if [ -z "$vv" ]; then
      bad "读不到 FollowPath.$ax"; cap_fail=1; continue
    fi
    printf '  MPPI %s = %s m/s\n' "$ax" "$vv"
    awk -v v="$vv" 'BEGIN{exit !(v<=0.2001)}' || { bad "$ax=$vv 超过授权的各轴上限 0.2"; cap_fail=1; }
  done
  if [ $cap_fail -eq 0 ]; then
    ok "各轴线速度上限 ≤ 0.2 m/s（合速度可达 0.283，按已定口径属正常）"
  else
    fail=1
  fi
...
# 按模长数会把合法的斜向运动（vx=vy=0.2 -> 模长 0.283）全报成越限。
axmax = max((max(abs(v[0]), abs(v[1])) for v in V), default=0.0)
over = sum(1 for v in V if abs(v[0]) > 0.2001 or abs(v[1]) > 0.2001)
print('  /cmd_vel %d 帧，非零 %d，单轴峰值 %.4f m/s（越 0.2 的帧 %d），'
      '合速度模长峰值 %.4f m/s（按轴口径下 ≤0.283 正常）'
      % (len(V), nz, axmax, over, vmax))
if over:
    print('  ✘ 有 %d 帧单轴超过 0.2 m/s —— 越过授权上限' % over); raise SystemExit(1)
...
say "阶段 6  nav2（MPPI，线速度上限 0.2 m/s）"
...
nohup setsid python3 $RL \
  ...
  controller_plugin:=mppi use_sim_time:=false max_linear_speed:=0.2 \
...
└─────────────────────────────────────────────────────────────────────────────┘
· 0.2 m/s 的口径：vx_max/vy_max 是**各轴独立**上限，合速度模长上界
  sqrt(0.2²+0.2²)=0.283，实测峰值 0.2085（1351 帧里 178 帧 > 0.2）。
  要把合速度压到 0.2 得两轴都给 0.141，或改成按模长限速。等口径确认。
...
日志都在 /tmp/s1_logs/：rsp jsp perception odom selfclear nav2 rviz explore bridge
TAIL
