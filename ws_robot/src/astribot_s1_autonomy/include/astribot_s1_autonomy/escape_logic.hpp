...
//      本机 inflation 把墙面向内吃掉一个内切半径 0.310m，机器人中心一旦进这条带，
...
//      consider_footprint 下足迹代价在窄于 1.24m 的通道里恒为 253、零梯度，
...
struct EscapeSearchConfig
{
  ...
  /// 0.31*sqrt(2)，即车体任一角点到中心的距离。
  ///
  /// 为什么必须有：只查中心线等于把机器人当质点。中心线离墙 0.05m 时中心线
  /// 全程「空闲」，但轮子已经压在墙上。这是本层最初实现的一个真实缺口。
  ///
  /// 为什么是外接半径（2026-09-07 由 0.386 改上来）：
  ///   本层用「圆盘沿线段扫掠」近似车体，而车体已从八边形改成正方形。
  ///   0.386 是八边形时代的实测轮系包络，对正方形**低估 0.052m**
  ///   （0.4384-0.386）—— 恰好是四个角伸出去的那一截。
  ///   低估方向 = 判成「没撞」而实际会撞，正是这条红线要防的那一侧。
  ///   旧注释的理由（"外接圆会把边中点附近本来能过的地方判成撞，
  ///   会让脱困在窄处无解"）本身没错，但那是**可用性**代价，而本层是红线层：
  ///   宁可无解转 PAUSED 等人工（有超时兜底），不可判错后撞上去。
  ///   要同时拿回那部分可用性，正解是把圆盘扫掠换成真正的正方形足迹扫掠，
  ///   而不是把半径调小到覆盖不住角点。
  ///
  /// !!! 改动此值必须同步 nav2 的 footprint !!!
  /// 这个数与 nav2_params_*.yaml 的 footprint 描述的是同一个车体，
  /// 由 astribot_s1_navigation 的 test_config_consistency.py 跨包钉住
  /// （那条测试从 nav2 的 yaml 现算外接半径，不抄本文件的常数 ——
  ///  否则"同源常量 + 同源断言"会把错值锁成期望行为）。
  ///
