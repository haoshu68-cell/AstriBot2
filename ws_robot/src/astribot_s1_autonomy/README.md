...
- 一个前沿格都没有 **且地图已知格 >= `min_known_cells_for_decision`** ⇒ `COMPLETED`（真的探索完了）
- 一个前沿格都没有 **但地图几乎全是未知** ⇒ 还**没开始**，走 `BOOTSTRAP`，不是完成
- 有前沿格但候选点全不合法 ⇒ `PAUSED`（只是暂时找不到合法通路）

第二条是实测踩出来的：`mapReady()` 查的是**消息层**（收到了、自洽、不超时），
冷启动时它全部通过，而地图里一个已知格都没有 —— 于是前沿格数也是 0，
只看前沿格数会把「还没开始」判成「探索完成」。
...
- `BOOTSTRAP`：slam_toolbox 只在机器人移动超过 `minimum_travel_heading`(本项目 0.2rad)
  之后才插入新扫描。冷启动时地图没有已知格 → 协调器不下发目标 → 机器人不动 →
  地图不长 → **永远不动**。以前每次冷启动都要人工发 `cmd_vel` 推一把（实测地图 0 → 26 m²）。
  这个状态把「推一把」收进节点自己：

  | 约束 | 取值 / 做法 | 为什么 |
  |---|---|---|
  | 只旋转，不平移 | `bootstrap_mode: rotate` | 足迹是外接 0.42 / 内切 0.388 的正八边形，原地转只扫过 3.2cm 环带；平移是开环积分推进，风险面完全不同 |
  | 速度发到 `/cmd_vel_nav_body_raw` | 不是 `/cmd_vel` | 保留 smoother → 倾倒监控 → 臂-底盘耦合限速 → 力矩闭环 leash 四层保护 |
  | 独立 20Hz 定时器重发 | 不靠 0.5s 状态机节拍 | 底盘 `cmd_vel_timeout_sec=0.5`，用状态机节拍发正好卡在超时边界上，速度被反复归零 |
  | 安全门持续生效 | 激光新鲜 + 最近障碍 ≥ 0.42m | **无数据不得视为安全**（本项目在探针脚本上踩过：RELIABLE 收不到 BEST_EFFORT 的 scan，代码把"无数据"当"前方无障碍"，盲走 3m） |
  | 离开状态必发零速 | 在 `transitionTo` 里统一做 | 一处覆盖所有出口（人工 pause / 安全门中断 / 正常转完 / 非法转换回退），漏一条就是"已暂停但机器人还在转" |
  | 实测转角，不看指令 | `bootstrap_min_yaw_delta: 0.20` | 指令发出去 ≠ 机器人动了。耦合节点最坏会把 0.40rad/s 缩到 0.06rad/s；达不到阈值时显式报错并指出这个原因 |
  | 有限次 | `bootstrap_max_attempts: 6`，成功下发目标后清零 | 禁止死循环重试 |

  **实测（仿真，强制触发 6 次）**：每次转出 1.353 / 1.417 / 1.457 / 1.479 / 1.452 / 1.471 rad，
  即下游总缩放约 0.85（名义 0.40×4.0=1.6rad），远好于按最坏限速 0.15 估的 0.24rad；
  6 次后正确停下等人工，自举窗口内 `controller_server` 零活动（无速度源打架）。

### 跟踪期什么时候才换路径

`follow_path` 下发模式下 BT 的 1Hz 重规划拿不到，得本节点自己补。
**但不能无条件周期重规划** —— 实测那样做的代价是：

> 36 个目标共下发 393 次 `FollowPath`，平均每个目标换 **10.9 条路径**，
> 新路径节拍约 1.5s。也就是说**没有任何一条路径被跟踪到位过**。

所以判据是 `replan_policy: on_invalid`：默认让控制器把当前路径跟完，只有下面之一成立才换：

| 触发条件 | 说明 |
|---|---|
| 控制器已中止 | 当前路径事实上作废（`replan_forced_`） |
| 剩余段不可通行 | 只校验**剩余段**，不是整条 —— 身后新观测到的障碍与"还能不能继续往前"无关 |
| 偏离超过 `path_deviation_limit_m` | 这条路径不再描述机器人的处境 |
| 超过 `path_max_age_sec` | 纯兜底，默认 0=不启用 |

两个必须守住的耦合（配错会把这个修复完全抵消，节点会**拒绝启动**）：

1. `path_deviation_limit_m` 必须 **>** `arrival_xy_tolerance`。否则收尾阶段的正常贴合误差
   被判成"已偏离"，每次快到目标时都换一条新路径 —— 现象与修复前一模一样。
2. 剩余段在接近终点时会退化成 1 个点，而 `validatePath` 对 <2 点的路径一律判不合法
   （这是对的）。直接拿它的结论决定换不换路径，会打断每个目标的收尾。

还有一个 **不能沿用已判死路径** 的规则：
"剩余段不可通行 → 重规划 → 新路径也不合法 → 沿用当前路径"这条兜底会让机器人
明知走不通还继续往里顶。实测顶了 **17 秒**，直到 `progress checker` 才救回来。
所以连续 `max_invalid_replan_attempts`(默认 3) 次拿不到合法替代就撤掉目标、
交给状态机另选一个前沿点。
