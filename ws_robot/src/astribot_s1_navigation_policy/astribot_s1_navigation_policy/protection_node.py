...
from .observer_node import rotate
...
class FinalProtection(Node):
    ...
    def tick(self):
        ...
        stop=not fresh or not lease or m.hold
        if fresh and (swept_point_collision(self.points,self.command,p) or
                      swept_point_collision(self.points,self.measured,p)):
            stop=True;reason='INDEPENDENT_SWEEP_RISK'
        if stop:self.clear_at=None
        else:
            if self.clear_at is None:self.clear_at=wall
            if wall-self.clear_at<p.clear_hold_s:stop=True;reason='PROTECTION_CLEAR_CONFIRMATION'
