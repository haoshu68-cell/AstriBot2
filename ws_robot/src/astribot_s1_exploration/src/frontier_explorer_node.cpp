...
void FrontierExplorerNode::declareParameters()
{
  ...
  declare_parameter<int>("completion_confirmations", 3);
  declare_parameter<double>("completion_stable_sec", 2.0);
...
bool FrontierExplorerNode::loadParameters(std::string & error)
{
  ...
  if (confirmations < 2 || confirmations > 1000 || !std::isfinite(completion_stable_sec_) ||
    completion_stable_sec_ <= 0.0) {error = "完成确认参数非法"; return false;}
  ...
  const GridMap & map, double robot_x, double robot_y, FrontierSearch::Result & out)
{
  ...
  if (out.status != FrontierSearch::Status::kOk || out.best_candidate_index >= 0) {
    return 0;
  }
...
void FrontierExplorerNode::planOnce()
{
  ...
  if (map_age > map_timeout_sec_) {
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), kLogThrottleMs,
      "地图已 %.1fs 未更新(超时阈值 %.1fs)，暂停输出目标，请检查 SLAM 是否存活",
  ...
  uint64_t revision;
  bool map_changed;
  {std::lock_guard<std::mutex> lock(map_mutex_); revision = map_revision_; map_changed = latest_map_ != map;}
  const bool fresh = std::isfinite(map_age) && map_age >= 0.0 && map_age <= map_timeout_sec_;
  if (result.status != FrontierSearch::Status::kOk || map_changed || !fresh) {
    completion_tracker_.reset();
    exploration_complete_ = false;
    std_msgs::msg::Bool done; done.data = false; complete_pub_->publish(done);
    publishStatus(kStateWaitingMap, "搜索失败或地图变化/过期，等待有效快照");
    return;
  }
  if (result.raw_frontier_cell_count == 0U) {
    const double seconds = std::chrono::duration<double>(
      std::chrono::steady_clock::now().time_since_epoch()).count();
    const bool eligible = result.unknown_cell_count == 0;
    exploration_complete_ = completion_tracker_.observe(revision, seconds, eligible,
      completion_observations_, completion_stable_sec_);
    std_msgs::msg::Bool done; done.data = exploration_complete_; complete_pub_->publish(done);
    publishStatus(exploration_complete_ ? kStateComplete : kStateNoValidGoal,
      !eligible ? "UNOBSERVABLE_UNKNOWN" : (exploration_complete_ ? "CURRENT_MAP_COMPLETE" : "CONFIRMING_COMPLETE"));
    return;
  }
  completion_tracker_.reset();
