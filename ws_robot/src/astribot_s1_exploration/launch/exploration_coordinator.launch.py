...
def _build_nodes(context, *args, **kwargs):
    ...
    bt_path = '' if bt_arg == 'default' else bt_arg
...
def generate_launch_description():
    ...
    declare_args = [
        ...
        DeclareLaunchArgument(
            'nav_behavior_tree', default_value='',
            description='留空或 default 使用导航栈的默认策略行为树；'
                        '自定义树必须保留 PolicyExecution 和 KeepSafePath session 契约。'),
    ]
