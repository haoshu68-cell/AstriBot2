  ...
  node_start_time_(0, 0, RCL_ROS_TIME)
{
  ...
  if (!loadParameters(error)) {
    throw std::invalid_argument("invalid perception configuration: "+error);
  }
  ...
  config_event_sub_=create_subscription<rcl_interfaces::msg::ParameterEvent>(
    "/parameter_events",rclcpp::ParameterEventsQoS(),
    [this](rcl_interfaces::msg::ParameterEvent::ConstSharedPtr event) {
      if(event->node==get_fully_qualified_name()) {config_dirty_.store(true);}
    });
...
void PointcloudSliceScanNode::declareParameters()
{
  ...
  const auto declared_slices = get_parameter("slice_names").as_string_array();
  for (const auto & name : declared_slices) {
    ...
    if (!has_parameter(prefix + "z_min")) {
      ...
      declare_parameter<double>(prefix + "max_range", get_parameter("scan.range_max").as_double());
      declare_parameter<bool>(prefix + "enabled", true);
    }
  }
  const auto declared_chains = get_parameter("self_filter.chain_names").as_string_array();
  for (const auto & name : declared_chains) {
...
PointcloudSliceScanNode::Configuration PointcloudSliceScanNode::parseConfiguration(
  const std::vector<rclcpp::Parameter> & updates) const
{
  std::map<std::string,rclcpp::Parameter> values;
  for(const auto & p:get_parameters(list_parameters({},100).names)) {values.emplace(p.get_name(),p);}
  for(const auto & p:updates) {values.insert_or_assign(p.get_name(),p);}
  for(const auto & item:values) {
    const auto & p=item.second;
    if(p.get_type()==rclcpp::ParameterType::PARAMETER_DOUBLE && !std::isfinite(p.as_double())) {
      throw std::invalid_argument("nonfinite parameter: "+item.first);
    }
    if(p.get_type()==rclcpp::ParameterType::PARAMETER_INTEGER &&
       (p.as_int()>std::numeric_limits<int>::max() || p.as_int()<std::numeric_limits<int>::min())) {
      throw std::invalid_argument("integer parameter out of range: "+item.first);
    }
  }
  auto parameter=[&](const std::string & name)->const rclcpp::Parameter & {return values.at(name);};
  Configuration candidate;std::string error;
  candidate.input_cloud_topic_ = parameter("input_cloud_topic").as_string();
  candidate.output_scan_topic_ = parameter("output_scan_topic").as_string();
  candidate.marker_topic_ = parameter("marker_topic").as_string();
  candidate.base_frame_ = parameter("base_frame").as_string();
  ...
  candidate.tf_total_budget_sec_ = parameter("tf_total_budget_sec").as_double();
  candidate.max_cloud_age_sec_ = parameter("max_cloud_age_sec").as_double();
  candidate.tf_time_tolerance_sec_ = parameter("tf_time_tolerance_sec").as_double();
  if (candidate.tf_timeout_sec_ < 0.0 || candidate.max_cloud_age_sec_ <= 0.0 || candidate.tf_time_tolerance_sec_ <= 0.0) {
    error = "tf_timeout_sec 需 >=0，max_cloud_age_sec / tf_time_tolerance_sec 需 >0";
    throw std::invalid_argument(error);
  }
  if (candidate.tf_total_budget_sec_ < 0.0) {
    error = "tf_total_budget_sec 需 >=0（0 表示所有查询都用 0 超时，只吃缓存）";
    throw std::invalid_argument(error);
  }
  if (candidate.tf_total_budget_sec_ > 0.0 && candidate.tf_total_budget_sec_ < candidate.tf_timeout_sec_) {
    error = "tf_total_budget_sec(" + std::to_string(candidate.tf_total_budget_sec_) +
      ") 小于 tf_timeout_sec(" + std::to_string(candidate.tf_timeout_sec_) +
      ")，单次查询永远等不满，等于静默改小了后者";
    throw std::invalid_argument(error);
  }

  candidate.enable_voxel_filter_ = parameter("enable_voxel_filter").as_bool();
  candidate.voxel_leaf_size_ = parameter("voxel_leaf_size").as_double();
  if (candidate.enable_voxel_filter_ && !(candidate.voxel_leaf_size_ > 0.0)) {
    error = "启用体素滤波时 voxel_leaf_size 必须为正";
    throw std::invalid_argument(error);
  }
  candidate.enable_outlier_filter_ = parameter("enable_outlier_filter").as_bool();
  candidate.outlier_mean_k_ = static_cast<int>(parameter("outlier_mean_k").as_int());
  candidate.outlier_stddev_mul_ = parameter("outlier_stddev_mul").as_double();
  if (candidate.enable_outlier_filter_ && (candidate.outlier_mean_k_ < 2 || !(candidate.outlier_stddev_mul_ > 0.0))) {
    error = "启用离群点滤波时要求 outlier_mean_k>=2 且 outlier_stddev_mul>0";
    throw std::invalid_argument(error);
  }

  candidate.min_valid_points_ = static_cast<int>(parameter("min_valid_points").as_int());
  if (candidate.min_valid_points_ < 0) {
    error = "min_valid_points 不能为负";
    throw std::invalid_argument(error);
  }
  candidate.watchdog_period_sec_ = parameter("watchdog_period_sec").as_double();
  candidate.input_timeout_sec_ = parameter("input_timeout_sec").as_double();
  if (candidate.input_timeout_sec_ <= 0.0) {
    error = "input_timeout_sec 必须为正";
    throw std::invalid_argument(error);
  }

  const std::string policy = parameter("invalid_input_policy").as_string();
  candidate.hold_last_max_frames_ = static_cast<int>(parameter("hold_last_max_frames").as_int());
  if (policy == "hold_last") {
    candidate.invalid_input_policy_ = InvalidInputPolicy::kHoldLast;
  } else if (policy == "stop_output") {
    candidate.invalid_input_policy_ = InvalidInputPolicy::kStopOutput;
  } else {
    error = "invalid_input_policy 只能是 hold_last 或 stop_output，当前=" + policy;
    throw std::invalid_argument(error);
  }

  candidate.publish_markers_ = parameter("publish_markers").as_bool();
  candidate.marker_point_stride_ = static_cast<int>(parameter("marker_point_stride").as_int());
  if (candidate.marker_point_stride_ < 1) {
    error = "marker_point_stride 必须 >=1";
    throw std::invalid_argument(error);
  }

  const std::vector<std::string> slice_names =
    parameter("slice_names").as_string_array();
  ...
  proj_params.angle_min = parameter("scan.angle_min").as_double();
  proj_params.angle_max = parameter("scan.angle_max").as_double();
  proj_params.angle_increment = parameter("scan.angle_increment").as_double();
  proj_params.range_min = parameter("scan.range_min").as_double();
  proj_params.range_max = parameter("scan.range_max").as_double();
  const std::string no_return_mode = parameter("scan.no_return_mode").as_string();
  ...
  for (const std::string & name : slice_names) {
    ...
    SliceConfig cfg;
    cfg.name = name;
    cfg.z_min = parameter(prefix + "z_min").as_double();
    cfg.z_max = parameter(prefix + "z_max").as_double();
    cfg.min_points = static_cast<int>(parameter(prefix + "min_points").as_int());
    cfg.max_range = parameter(prefix + "max_range").as_double();
    cfg.enabled = parameter(prefix + "enabled").as_bool();
    proj_params.slices.push_back(cfg);
  }

  std::string proj_error;
  if (!candidate.projector_.configure(proj_params, proj_error)) {
    error = "切片投影器参数非法: " + proj_error;
    throw std::invalid_argument(error);
  }

  FootprintCylinder footprint;
  footprint.enabled = parameter("self_filter.footprint.enabled").as_bool();
  footprint.radius = parameter("self_filter.footprint.radius").as_double();
  footprint.z_min = parameter("self_filter.footprint.z_min").as_double();
  footprint.z_max = parameter("self_filter.footprint.z_max").as_double();
  ...
  candidate.self_filter_.setFootprint(footprint);

  candidate.self_filter_chains_.clear();
  const std::vector<std::string> chain_names =
    parameter("self_filter.chain_names").as_string_array();
  for (const std::string & name : chain_names) {
    const std::string prefix = "self_filter.chains." + name + ".";

    SelfFilterChainConfig chain;
    chain.name = name;
    chain.radius = parameter(prefix + "radius").as_double();
    chain.frames = parameter(prefix + "frames").as_string_array();
    if (!(chain.radius > 0.0)) {
    ...
    candidate.self_filter_chains_.push_back(std::move(chain));
  }

  candidate.scan_time_=parameter("scan.scan_time").as_double();
  if(candidate.scan_time_<=0) {throw std::invalid_argument("scan.scan_time must be positive");}
  return candidate;
}
...
{
  try {
    auto candidate=parseConfiguration({});
    std::lock_guard<std::recursive_mutex> lock(config_mutex_);
    input_cloud_topic_=std::move(candidate.input_cloud_topic_);
    ...
    tf_time_tolerance_sec_=std::move(candidate.tf_time_tolerance_sec_);
    enable_voxel_filter_=std::move(candidate.enable_voxel_filter_);
    voxel_leaf_size_=std::move(candidate.voxel_leaf_size_);
    enable_outlier_filter_=std::move(candidate.enable_outlier_filter_);
    outlier_mean_k_=std::move(candidate.outlier_mean_k_);
    outlier_stddev_mul_=std::move(candidate.outlier_stddev_mul_);
    min_valid_points_=std::move(candidate.min_valid_points_);
    watchdog_period_sec_=std::move(candidate.watchdog_period_sec_);
    input_timeout_sec_=std::move(candidate.input_timeout_sec_);
    publish_markers_=std::move(candidate.publish_markers_);
    marker_point_stride_=std::move(candidate.marker_point_stride_);
    invalid_input_policy_=std::move(candidate.invalid_input_policy_);
    hold_last_max_frames_=std::move(candidate.hold_last_max_frames_);
    self_filter_chains_=std::move(candidate.self_filter_chains_);
    projector_=std::move(candidate.projector_);
    self_filter_=std::move(candidate.self_filter_);
    scan_time_=std::move(candidate.scan_time_);
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  static const std::vector<std::string> kRestartRequired = {
    ...
    "filtered_cloud_topic", "slice_names", "self_filter.chain_names"};
  ...
  try {(void)parseConfiguration(parameters);}
  catch(const std::exception & e) {result.successful=false;result.reason=e.what();return result;}
