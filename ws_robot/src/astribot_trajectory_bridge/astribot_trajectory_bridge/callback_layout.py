...
#:
#: !!! cmd 必须与 inner 分开 !!!
#: 内环每拍要做两次跨进程 SDK 往返（判 leash 的读 + 下发的写），实测周期
#: 6.0~14.9ms，撑不到 freq=250 要求的 4ms。只要平均周期 > 4ms，定时器回调就
#: 永久积压把组占满，同组的订阅回调一次都拿不到执行机会。
CHASSIS_GROUPS = ('inner', 'outer', 'srv', 'cmd')

#: 机械臂桥接节点的回调组。
#: 'exec' 是**可重入**组（cancel 回调要能在 execute 还在跑的时候被处理），
#: 其余是互斥组。可重入组不串行，但仍然要占线程才能真正并发，所以一样计数。
ARM_GROUPS = ('exec', 'srv', 'grip')

#: 留给 executor 自身的余量（参数事件、时钟、以及我们没显式指定组的回调
#: 会落到节点默认组）。
THREAD_HEADROOM = 2
