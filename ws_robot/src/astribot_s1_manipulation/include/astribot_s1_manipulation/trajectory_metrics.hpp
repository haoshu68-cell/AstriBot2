...
// 为什么要自己算而不信规划器的输出
// --------------------------------
// OMPL 出来的是**纯几何路径**，没有时间信息（所有 waypoint 的时间戳都是 0）。
// 时间是后面 IPTP/TOTG 打上去的。任务要求量化"总时长/最大关节速度/加速度"
// 并据此判断轨迹是否超限，所以必须在时间参数化**之后**独立算一遍：
//   · 不能信任时间参数化器"应该"遵守了限位 —— 实测 TOTG 在路径点过密
//     或存在重复点时会给出超限的结果，必须自己复核。
//   · 任务明确要求"优化后仍超限则回退原始轨迹"，判据就来自这里。
//
// 速度/加速度怎么取
// ----------------
// RobotTrajectory 的每个 waypoint 是一个 RobotState，时间参数化器会把
// 关节速度/加速度写进 RobotState（hasVelocities/hasAccelerations）。
// 优先直接读它们（这才是控制器实际会收到的值）。
// 如果没有（例如未参数化的原始路径），退化为按位置差分估算，并在报告里标明
// 数据来源 —— 差分值和参数化器写入的值可能有出入，混着看会得出错误结论。
