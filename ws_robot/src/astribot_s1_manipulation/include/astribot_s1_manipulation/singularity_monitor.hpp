...
struct SingularityParams
{
  ...
  ///
  /// 为什么默认 true —— 这是 Gazebo 实测逼出来的结论：
  /// 起点是"机器人当前所在的位置"，不是我们挑的。仿真里机器人就是以全零构型
  /// 出生的，而本机器人全零即肘部完全伸直 —— 精确奇异
  /// （实测 joint_4=0.0008、sigma_min=0.0077 < 阈值 0.02）。
  /// 如果连起点都拒，那么从奇异构型出发的**任何**轨迹都会被否决，
  /// 机器人被永久锁死在奇异构型里，连"挪出去"这唯一出路都被堵上。
  ///
  /// 正确语义：奇异检测要阻止的是"轨迹**走进**奇异构型"，
  /// 而不是"轨迹从奇异构型**离开**"。
  ///
  /// 实现规则（不是只豁免第 0 点 —— 实测那样不够）：
  /// 在**首次出现非奇异点之前**的所有点都算"逃离段"、一律豁免；
  /// 一旦出现过非奇异点就说明已脱离奇异邻域，之后任何奇异点都判否决。
  /// 因为从奇异点出发时头几个点必然还在奇异邻域里 —— 实测全零起步时
  /// waypoint 1 的 sigma_min 仍只有 0.0104（阈值 0.02），只豁免第 0 点
  /// 依然会被卡住。这个规则自带终止性，不需要配豁免点数上限。
  /// 豁免点的奇异值仍会被测量并写进报告（只是不据此否决）。
  ///
  /// 设 false 则连起点也严格拒绝 —— 只在"确信机器人不会停在奇异构型"的
  /// 场景下才这么配。
